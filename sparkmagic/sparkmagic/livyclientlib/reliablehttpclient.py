        ...
        while True:
            try:
                if self._endpoint.auth == constants.NO_AUTH:
                    ...
                    if data is None:
                        r = function(url, headers=self._headers, auth=self._auth, verify=self.verify_ssl, cookies = self._endpoint.cookies)
                    else:
                        r = function(url, headers=self._headers, auth=self._auth, data=json.dumps(data), verify=self.verify_ssl, cookies = self._endpoint.cookies)
