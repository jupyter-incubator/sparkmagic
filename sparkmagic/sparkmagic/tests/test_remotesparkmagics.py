...
from sparkmagic.utils.constants import EXPECTED_ERROR_MSG, BASIC_AUTH
...
def test_add_sessions_command_parses():
    ...
    name = "-s name"
    language = "-l python"
    connection_string = "-u http://url.com --authentication {} -a sdf -p w".format(BASIC_AUTH)
    line = " ".join([command, name, language, connection_string])

    magic.spark(line)

    add_sessions_mock.assert_called_once_with("name", Endpoint("http://url.com", BASIC_AUTH, "sdf", "w"),
                                              False, {"kind": "pyspark"})
...
def test_delete_sessions_command_parses():
    ...
    command = "delete -u URL --authentication {} -a username -p password -i 4".format(BASIC_AUTH)
    mock_method = MagicMock()
    spark_controller.delete_session_by_id = mock_method
    magic.spark(command)
    mock_method.assert_called_once_with(Endpoint("URL", BASIC_AUTH, "username", "password"), 4)
...
def test_cleanup_endpoint_command_parses():
    ...
    line = "cleanup -u endp --authentication {} -a user -p passw".format(BASIC_AUTH)
    magic.spark(line)
    mock_method.assert_called_with(Endpoint("endp", BASIC_AUTH, "user", "passw"))
