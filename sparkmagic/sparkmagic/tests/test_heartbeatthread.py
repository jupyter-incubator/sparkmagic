...
def test_run_once():
    ...
    session.refresh_status_and_log.assert_called_once_with()
...
def test_run_stops():
    ...
    assert session.refresh_status_and_log.called
    assert heartbeat_thread.livy_session is None
...
def test_run_retries():
    msg = "oh noes!"
    session = MagicMock()
    session.refresh_status_and_log = MagicMock(side_effect=ValueError(msg))
    refresh_seconds = 0.1
    retry_seconds = 0.1
    heartbeat_thread = _HeartbeatThread(session, refresh_seconds, retry_seconds, 1)
