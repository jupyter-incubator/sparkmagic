...
from nose.tools import with_setup, assert_equals, assert_in, assert_raises
import pandas as pd
...
def test_pyspark_livy_sampling_options():
    ...
    assert_in(".take(120)", sparkcommand._pyspark_command(variable_name).code)

    sparkcommand = SparkStoreCommand(variable_name, samplemethod='take', maxrows=-1)
    assert_in(".collect()", sparkcommand._pyspark_command(variable_name).code)

    sparkcommand = SparkStoreCommand(variable_name, samplemethod='sample', samplefraction=0.25, maxrows=-1)
    assert_in(".sample(False, 0.25).collect()", sparkcommand._pyspark_command(variable_name).code)

    sparkcommand = SparkStoreCommand(variable_name, samplemethod='sample', samplefraction=0.33, maxrows=3234)
    assert_in(".sample(False, 0.33).take(3234)", sparkcommand._pyspark_command(variable_name).code)

    sparkcommand = SparkStoreCommand(variable_name, samplemethod=None, maxrows=100)
    assert_in(".take(100)", sparkcommand._pyspark_command(variable_name).code)
