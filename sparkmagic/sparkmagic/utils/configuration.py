...
class SparkMagicConfiguration(Configuration):
    """Utility to read configs for sparkmagic.
    """
    def __init__(self):
        super(SparkMagicConfiguration, self).__init__(HOME_PATH, CONFIG_FILE)
    ...
    def base64_kernel_python_credentials(self):
        return self._credentials_override(self.kernel_python_credentials)       
    
    @Configuration._override
    def kernel_scala_credentials():
        return {u'username': u'', u'base64_password': u'', u'url': u'http://localhost:8998'}

    def base64_kernel_scala_credentials(self):        
        return self._credentials_override(self.kernel_scala_credentials)
    ...
    def _credentials_override(self, f):
